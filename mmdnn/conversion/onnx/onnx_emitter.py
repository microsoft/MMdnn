...
"""
    ...
    def emit_DataInput(self, IR_node):
        ...
        self.add_body(1, "{:<15} = helper.make_tensor_value_info('{}', {}, ({},))".format(
            ...
            shape_str))
        self.add_body(1,
                      "{:15} = helper.make_node('Transpose', inputs=['{}'], outputs=['{}'], perm=[0, 3, 1, 2], name='{}')".format(
                          IR_node.variable_name,
                          IR_node.variable_name + '_orig',
                          IR_node.variable_name,
                          IR_node.variable_name
    ...
    def emit_Conv(self, IR_node):
        ...
        self.add_body(1, "{} = {}.transpose([3,2,0,1])".format(
            ...
            IR_node.variable_name + '_weight_array'))
        self.add_body(1,
                      "{:15} = helper.make_tensor_value_info('{}', onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], list({}.shape))".format(
        ...
        self.add_body(1,
                      "{:15} = helper.make_tensor(name='{}', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={}.flatten().astype(float))".format(
        ...
        if use_bias:
            ...
            self.add_body(1,
                          "{:15} = helper.make_tensor(name='{}', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={}.flatten().astype(float))".format(
                              ...
                              IR_node.variable_name + '_bias_array'))
            self.add_body(1,
                          "{:15} = helper.make_node('Conv', inputs=['{}', '{}', '{}'],outputs=['{}'], dilations={}, group={}, kernel_shape={}, pads={}, strides={}, name='{}')".format(
                              IR_node.variable_name,
    ...
    def emit_BatchNorm(self, IR_node):
        ...
        if IR_node.get_attr('scale'):
            ...
            self.add_body(1,
                          "{:15} = np.ndarray(__weights_dict['{}']['bias'].shape, dtype=__weights_dict['{}']['bias'].dtype).squeeze()".format(
                              IR_node.variable_name + '_scale_array',
                              IR_node.name,
                              IR_node.name))
        ...
        self.add_body(1,
                      "{:15} = helper.make_tensor(name='{}', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={})".format(
        ...
        self.add_body(1,
                      "{:15} = helper.make_node('BatchNormalization', inputs=['{}', '{}', '{}', '{}', '{}'],outputs=['{}'], epsilon={}, is_test={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name + '_scale',
                          IR_node.variable_name + '_bias',
    ...
    def emit_Pool(self, IR_node):
        ...
        if IR_node.layer.attr['global_pooling'].b:
            if pooling_type == 'AVG':
                self.add_body(1,
                              "{:15} = helper.make_node('GlobalAveragePool', inputs=['{}'], outputs=['{}'], name='{}')".format(
                                  IR_node.variable_name,
                                  self.parent_variable_name(IR_node),
                                  IR_node.variable_name,
                                  IR_node.variable_name))
            ...
            if pooling_type in ['AVG', 'MAX']:
                ...
                strides = list(IR_node.get_attr('strides')[1:-1])
                self.add_body(1,
                              "{:15} = helper.make_node('{}', inputs=['{}'],outputs=['{}'], kernel_shape={}, pads={}, strides={}, name='{}')".format(
                                  IR_node.variable_name,
                                  op_name,
                                  self.parent_variable_name(IR_node),
                                  IR_node.variable_name,
    ...
    def emit_FullyConnected(self, IR_node):
        ...
        self.add_body(1,
                      "{:15} = helper.make_tensor(name='{}', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={}.flatten().astype(float))".format(
                          ...
                          IR_node.variable_name + '_bias_array'))
        self.add_body(1,
                      "{:15} = helper.make_node('Gemm', inputs=['{}', '{}', '{}'], outputs=['{}'], name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name + '_weight',
                          IR_node.variable_name + '_bias',
    ...
    def emit_Pad(self, IR_node):
        ...
        self.add_body(1,
                      "{:15} = helper.make_node('Pad', inputs=['{}'], outputs=['{}'], mode='{}', pads={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          mode,
    ...
    def emit_Constant(self, IR_node):
        if IR_node.get_attr('value'):
            ...
            self.add_body(1, "{:15} = __weights_dict['{}']['value']".format(
                ...
                IR_node.name))
        self.add_body(1,
                      "{:15} = helper.make_node('Constant', inputs=[], outputs=['{}'], value=helper.make_tensor(name='const_tensor', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={}.flatten().astype(float)), name='{}')".format(
                          IR_node.variable_name,
                          IR_node.variable_name,
                          IR_node.variable_name + '_value_array',
                          IR_node.variable_name + '_value_array',
    ...
    def emit_Mul(self, IR_node):
        ...
        if IR_node.name in self.weights_dict and 'weights' in self.weights_dict[IR_node.name]:
            ...
            self.add_body(1,
                          "{:15} = helper.make_node('Constant', inputs=[], outputs=['{}'], value=helper.make_tensor(name='const_tensor', data_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[{}.dtype], dims={}.shape, vals={}), name='{}')".format(
                              IR_node.variable_name + '_weight',
                              IR_node.variable_name + '_weight',
                              IR_node.variable_name + '_weight_array',
                              IR_node.variable_name + '_weight_array',
    ...
    def emit_Dropout(self, IR_node):
        self.add_body(1,
                      "{:15} = helper.make_node('Dropout', inputs=['{}'], outputs=['{}'], is_test={}, ratio={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          0 if self.phase == 'train' else 1,
    ...
    def emit_ReduceMean(self, IR_node):
        ...
        self.add_body(1,
                      "{:15} = helper.make_node('ReduceMean', inputs=['{}'], outputs=['{}'], axes=[{}], keepdims={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          axes,
    ...
    def emit_LRN(self, IR_node):
        ...
        self.add_body(1,
                      "{:15} = helper.make_node('LRN', inputs=['{}'], outputs=['{}'], alpha={}, beta={}, bias={}, size={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          alpha,
    ...
    def emit_Relu6(self, IR_node):
        self.add_body(1,
                      "{:15} = helper.make_node('Clip', inputs=['{}'], outputs=['{}'], min=0.0, max=6.0, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          IR_node.variable_name))
    ...
    def emit_Slice(self, IR_node):
        if self.IR_graph.get_parent(IR_node.name, [0]).type == 'Shape':
            ...
            self.add_body(1,
                          "{:15} = helper.make_node('Slice', inputs=['{}'], outputs=['{}'], starts={}, ends={}, name='{}')".format(
                              IR_node.variable_name,
                              self.parent_variable_name(IR_node),
                              IR_node.variable_name,
                              starts,
    ...
    def emit_LeakyRelu(self, IR_node):
        alpha = IR_node.get_attr('alpha')
        self.add_body(1,
                      "{:15} = helper.make_node('LeakyRelu', inputs=['{}'], outputs=['{}'], alpha={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          alpha,
    ...
    def emit_SpaceToDepth(self, IR_node):
        ...
        self.add_body(1,
                      "{:15} = helper.make_node('SpaceToDepth', inputs=['{}'], outputs=['{}'], blocksize={}, name='{}')".format(
                          IR_node.variable_name,
                          self.parent_variable_name(IR_node),
                          IR_node.variable_name,
                          blocksize,
